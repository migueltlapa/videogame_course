...
struct Primitive
{
	Primitive(const std::shared_ptr<TriangleMesh> &m, const std::shared_ptr<MaterialObj> &mat) :
	mesh(m), material(mat) {}
	const std::shared_ptr<TriangleMesh> mesh;   /*! the object's geometry */
	const std::shared_ptr<MaterialObj> material;   /*! the object's material */
};
...
public:
	...
	std::map<std::string, std::shared_ptr<MaterialObj> > materials;
	std::shared_ptr<MaterialObj> curMaterial;
