...
void ObjReader::loadMTL(const std::string &mtlFilename)
{
    ...
    std::shared_ptr<MaterialObj> mat;
    while (ifs.peek() != EOF) {
        ...
        if (!strncmp(token, "newmtl", 6)) {
            ...
            materials[name] = mat = std::shared_ptr<MaterialObj>(new MaterialObj);
            continue;
        }
...
ObjReader::ObjReader(const char *filename)
{
    ...
    try {
        ...
        std::shared_ptr<MaterialObj> defaultMaterial(new MaterialObj);
...
{
    ObjReader objReader("./cornell_box.obj");
    return 0;
}*/
#endif