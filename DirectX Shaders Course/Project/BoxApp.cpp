...
BoxApp::BoxApp(HINSTANCE hInstance) : DXApp(hInstance)
{
	m_appTitle = "BoxApp";
...
bool BoxApp::Init()
{
	...
	XMMATRIX P = XMMatrixPerspectiveFovLH(XMConvertToRadians(45.0f), m_ClientWidth / m_ClientHeight, 0.1f, 100.0f);
...
void BoxApp::InitConstantBuffers(){
...
int WINAPI WinMain3(__in HINSTANCE hInstance, __in_opt HINSTANCE hPrevInstance, __in LPSTR lpCmdLine, __in int nShowCmd)
{
	BoxApp tApp(hInstance);
	if (!tApp.Init())
		return 1;
