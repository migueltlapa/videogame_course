...
void ModelApp::InitShaders(){
	...
	Utility::CompileShader(L"Shaders/ShaderModelV.hlsl", "VShader", profileV, &VS);
	Utility::CompileShader(L"Shaders/ShaderModelP.hlsl", "PShader", profileP, &PS);
...
void ModelApp::initPrecompiledShaders()
{
...
#if _DEBUG
	LPCWSTR compiledVShader = L"Debug/ShaderModelVd.cso";
	LPCWSTR compiledPShader = L"Debug/ShaderModelPd.cso";
#else
	LPCWSTR compiledVShader = L"Release/ShaderModelV.cso";
	LPCWSTR compiledPShader = L"Release/ShaderModelP.cso";
#endif
	...
	m_pDevice->CreateInputLayout(ied, 2,
...
int WINAPI WinMain5(__in HINSTANCE hInstance, __in_opt HINSTANCE hPrevInstance, __in LPSTR lpCmdLine, __in int nShowCmd)
{
