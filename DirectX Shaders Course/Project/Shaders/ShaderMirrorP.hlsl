...
float4 PShader(PInB IN) : SV_TARGET
{
	...
	float4 finalColor = (gMaterial.Ambient +
						gMaterial.Diffuse*lr.Diffuse)*texColor +
	...
	{
		float3 incident = -V;
			float3 reflectionVector = reflect(incident, N);
			float4 reflectionColor = gCubeMap.Sample(Sampler, reflectionVector);
	...
	if (gMaterial.EnableRefraction)
	{
		...
		float3 incident = -V;
			float3 refractionVector = refract(incident, N, gMaterial.Refract.a);
			float4 refractionColor = gCubeMap.Sample(Sampler, refractionVector);
			finalColor += float4(gMaterial.Refract.xyz,1.0f)* refractionColor;
	}
